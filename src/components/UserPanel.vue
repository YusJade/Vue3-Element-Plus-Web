<template>
  ...
  <el-descriptions title="个人面板" direction="vertical" :column="2" border>
    <el-descriptions-item label="用户名">
      ...
      <ValidatedInput v-else size="small" id="username" :clearable="false" :model-value="user.username"
        :validate="validateUsernameLogined">
      </ValidatedInput>
    </el-descriptions-item>
    <el-descriptions-item label="电话">
      <span v-if="!isEditState"> {{ userStore.userInfo.phone }} </span>
      <ValidatedInput v-else size="small" id="phone" :clearable="false" :model-value="user.phone"
        :validate="validatePhone" </ValidatedInput>
    </el-descriptions-item>
    <el-descriptions-item label="邮箱" :span="2">
      <span v-if="!isEditState"> {{ userStore.userInfo.email }} </span>
      <ValidatedInput v-else size="small" id="email" :clearable="false" :model-value="user.email"
        :validate="validateEmail">
      </ValidatedInput>
    </el-descriptions-item>
    <el-descriptions-item label="姓名">
      <span v-if="!isEditState"> {{ userStore.userInfo.name }} </span>
      <ValidatedInput v-else size="small" id="name" :clearable="false" :model-value="user.name">
      </ValidatedInput>
    </el-descriptions-item>
    <el-descriptions-item label="性别">
      <span v-if="!isEditState"> {{ userStore.userInfo.gender }} </span>
      <el-select v-else v-model="user.gender" size="middle" style="width: 240px" placeholder="性别">
